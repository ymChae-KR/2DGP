...
class VertScrollBackground:
    ...
    def draw(self):
        ...
        curr = int(-self.scroll) % page
        if curr > 0:
            sw = int(1 + self.image.w * curr / self.cw)
            sl = self.image.h - sw
            src = 0, sl, self.image.w, sw
            dw = int(sw * self.cw / self.image.w)
            dst = 0, curr - dw, self.cw, dw
            self.image.clip_draw_to_origin(*src, *dst)
        ...
        if curr < self.ch:
            dw = self.ch - curr
            sw = int(1 + self.image.w * dw / self.cw)
            src = 0, 0,  self.image.w, sw
            dw = int(sw * self.cw / self.image.w)
            dst =  0,curr, self.ch, dw
            self.image.clip_draw_to_origin(*src, *dst)
